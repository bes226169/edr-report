...
namespace EDR_Report.Controllers
{
    public partial class ReportController
    {
        ...
        /// <param name="cd"></param>
        /// <returns></returns>
        IActionResult RPT1CA803(string? state, string cd)
        {
            ...
            string filePath = $"{BasePath.TrimEnd('\\').TrimEnd('/')}\\edr1ca803.xlsx";
            if (!System.IO.File.Exists(filePath)) filePath = "report/edr1ca803.xlsx";
            if (!System.IO.File.Exists(filePath)) return NotFound("找不到套表檔");

            DBFunc db = new();
            // 讀入資料
            var projInfo = GetProjInfo_1ca803(db, projectId, calendarDate);
            var projConsOverview = GetProjConsOverview_1ca803(db, projectId, calendarDate, myuserId, state, false);
            ////////////
            // 編輯區 //
            ////////////
            if (projectId == 6040 || projectId == 5541 || projectId == 5782 || projectId == 4760)
            {
                projConsOverview = GetProjConsOverview_1ca803(db, projectId, calendarDate, myuserId, "1", false); //1:所有,2:本日
            }
            var projConsOverviewSpec = GetProjConsOverview_1ca803(db, projectId, calendarDate, myuserId, state, true);
            var projMaterial = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "2", "material"); //1:所有,2:本日
            var projMan = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "2", "man"); //1:所有,2:本日
            var projMachine = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "2", "machine"); //1:所有,2:本日
            if (projectId == 5782)
            {
                projMaterial = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "1", "material"); //1:所有,2:本日
                projMan = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "1", "man"); //1:所有,2:本日
                projMachine = GetProjManMachineMaterial_1ca803(db, projectId, calendarDate, "1", "machine"); //1:所有,2:本日
            }
            var projNote = GetProjNote_1ca803(db, projectId, calendarDate);
            var projMilestone = GetProjMilestone_1ca803(db, projectId);
            // 待確認資料來源
            ...
            Dictionary<string, object[]> variables = new()
            {
                ...
                { "$material_col1$", projMaterial["ROW_NUMBER"]},
            ...
            using (var fileStream = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                if (projectId == 6040)
                {// 依資料長度調整列數
                    AdjustRowNums_1ca803(ws, variables, "$co_col2$");
                    AdjustRowNums_1ca803(ws, variables, "$material_col2$");
                    AdjustRowNums_1ca803(ws, variables, "$man_col1$", "$machine_col1$");
                    // 調整高度，要先調列數再調高度不然後面shift上去的row高度會改為預設高度
                    AdjustRowHeight_1ca803(ws, variables, "$project_name$");
                    AdjustRowHeight_1ca803(ws, variables, "$co_col2$");
                    AdjustRowHeight_1ca803(ws, variables, "$material_col2$");
                    AdjustRowHeight_1ca803(ws, variables, "$man_col1$", "$machine_col1$");
                    AdjustRowHeight_1ca803(ws, variables, "$note$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_a$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_b$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_c$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_d$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_g$");
                }
                ...
                {
                    AdjustRowNums_1ca803(ws, variables, "$co_col2$");
                    AdjustRowNums_1ca803(ws, variables, "$man_col1$", "$machine_col1$", "$material_col2$");
                    // 調整高度，要先調列數再調高度不然後面shift上去的row高度會改為預設高度
                    AdjustRowHeight_1ca803(ws, variables, "$project_name$");
                    AdjustRowHeight_1ca803(ws, variables, "$co_col2$");
                    AdjustRowHeight_1ca803(ws, variables, "$man_col1$", "$machine_col1$", "$material_col2$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_a$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_c$");
                    AdjustRowHeight_1ca803(ws, variables, "$note_d$");
                }
                ...
                {
                    //AdjustRowNumsAndHeight_1ca803(ws, variables, "$period1$");
                    //AdjustRowNumsAndHeight_1ca803(ws, variables, "$period5$");
                    //VerticalMergeRows_1ca803(ws, "$period1$", new int[] { 1, 9, 17, 25 });
                    //VerticalMergeRows_1ca803(ws, "$period5$", new int[] { 1, 9, 17, 25 });
                }

                // 替換資料
                ImportDataIntoPlaceholder_1ca803(ws, variables);
            }
            // 不留實體檔案，直接串流到前端
        ...
        // 一、資料模型
        public class PROJECT_INFO_MODEL_1ca803
        {
        ...
        public class PROJECT_MMM_MODEL_1ca803
        {
            public string? PROJECT_ID { get; set; }
        ...
        public class PROJECT_NOTE_MODEL_1ca803
        {
        ...
        public class PROJECT_CONSTRUCTION_OVERVIEW_MODEL_1ca803
        {
        ...
        public class PROJECT_MILESTONE_MODEL_1ca803
        {
            public string? PROJECT_ID { get; set; }
            public string? MILESTONE_NO_SUB { get; set; }
            public string? MILESTONE_WK_DAY { get; set; }
        }
        public class FORM_DATA_MODEL_1ca803
        {
            public int SelectedOption { get; set; }
            public DateTime CalendarDate { get; set; }
        }
        // 二、資料字典
        static Dictionary<string, object[]> ConvertListToDictionary_1ca803<T>(List<T> list)
        {
        ...
        static Dictionary<string, object[]> GetProjInfo_1ca803(DBFunc db, int? projectId, DateTime calendarDate)
        {
            string calendarDateStr = calendarDate.ToString("yyyy/MM/dd");
            var list = db.query<PROJECT_INFO_MODEL_1ca803>("edr", @"
            SELECT
                '112A040C009-' || 
            ...
            if (list.Count == 0)
            {
                list = db.query<PROJECT_INFO_MODEL_1ca803>("edr", @"
                    SELECT
                          ' ' AS PROJECT_NAME
                        , ' ' AS PROJECT_ID
                        , ' ' AS PROJECT_OWN_CH
            ...
            return ConvertListToDictionary_1ca803(list);
        }
        static Dictionary<string, object[]> GetProjConsOverview_1ca803(DBFunc db, int? projectId, DateTime calendarDate, int? myuserId, string myState, bool specific = false)
        {
            ...
            string query3 = $@"
                ...
                {query3_}
                ";
            var list = db.query<PROJECT_CONSTRUCTION_OVERVIEW_MODEL_1ca803>("edr", query3).ToList();
            return ConvertListToDictionary_1ca803(list);
        }
        static Dictionary<string, object[]> GetProjManMachineMaterial_1ca803(DBFunc db, int? projectId, DateTime calendarDate, string state, string resource)
        {
            string calendarDateStr = calendarDate.ToString("yyyy/MM/dd");
            ...
            var list = db.query<PROJECT_MMM_MODEL_1ca803>("edr", $@"
                SELECT       
                      PROJECT_ID
                    , RESOURCE_CLASS
                    , OWN_RES_CODE AS PBG_CODE
                    , SEQUENCE_NO
                    , ROW_NUMBER() OVER (ORDER BY OWN_RES_CODE) AS ROW_NUMBER
                    , MAX(NAME) AS NAME
                    , TRIM(MAX(UNIT)) AS UNIT
                ...
                FROM (
                    SELECT a.*, b.SEQUENCE_NO FROM vsuser.bes_edr_resqty_v a
                    LEFT JOIN (
                        SELECT 
                              RESOURCE_ID
                            , SEQUENCE_NO 
                        FROM vsuser.bes_project_resources
                ...
                GROUP BY PROJECT_ID, RESOURCE_CLASS, OWN_RES_CODE, SEQUENCE_NO
                HAVING SUM(CASE WHEN DATA_DATE = TO_DATE(:calendarDateStr, 'yyyy/MM/dd') THEN NVL(QUANTITY, 0) ELSE 0 END) {queryRange}
                ORDER BY RESOURCE_CLASS, SEQUENCE_NO
                ", new
            {
                projectId,
                calendarDateStr
        ...
        static Dictionary<string, object[]> GetProjNote_1ca803(DBFunc db, int? projectId, DateTime calendarDate)
        {
            string calendarDateStr = calendarDate.ToString("yyyy/MM/dd");
            var list = db.query<PROJECT_NOTE_MODEL_1ca803>("edr", $@"
                SELECT
                    project_id
            ...
            if (list.Count == 0)
            {
                list = db.query<PROJECT_NOTE_MODEL_1ca803>("edr", $@"
                    SELECT
        ...
        static Dictionary<string, object[]> GetProjMilestone_1ca803(DBFunc db, int? projectId)
        {
            var list = db.query<PROJECT_MILESTONE_MODEL_1ca803>("edr", $@"
                SELECT 
                      PROJECT_ID
        ...
        static void WriteCellValueByPlaceholder_1ca803(ISheet sheet, string placeholder, object cellValue)
        {
        ...
        static void WriteCellValueByIndex_1ca803(ISheet sheet, int rowIndex, int colIndex, object cellValue)
        {
            rowIndex--;
            colIndex--;
            IRow row = sheet.GetRow(rowIndex);
        ...
        static int[,] GetCellIndexByPlaceholder_1ca803(ISheet sheet, string placeholder)
        {
        ...
        static void RemoveRangeRows_1ca803(ISheet sheet, int rowIndexStart, int rowIndexEnd)
        {
            rowIndexStart--;
            rowIndexEnd--;
        ...
        static void CopyRowDown_1ca803(ISheet sheet, int rowIndexStart, int repeatTimes)
        {
            rowIndexStart--;
        ...
        // 依範圍合併儲存格，index從1開始
        static void MergeCells_1ca803(ISheet sheet, int startRow, int startColumn, int endRow, int endColumn)
        {
            if (sheet == null)
            {
        ...
        static string[] FindPlaceholderAppearingTimes_1ca803(ISheet sheet, int times)
        {
        ...
        static void ReplaceAllSinglePlaceholder_1ca803(ISheet sheet, Dictionary<string, object[]> data)
        {
            string[] uniquePlaceholder = FindPlaceholderAppearingTimes_1ca803(sheet, 1);
            foreach (string placeholder in uniquePlaceholder)
            {
                if (data.ContainsKey(placeholder))
                {
                    ...
                    {
                        WriteCellValueByPlaceholder_1ca803(sheet, placeholder, data[placeholder][0]);
                    }
                    else if (data[placeholder].Length == 0)
                    {
                        WriteCellValueByPlaceholder_1ca803(sheet, placeholder, "");
                    }
                }
            }
        }
        // 取代資料出現1次以上的placeholder
        static void ReplaceMultiplePlaceholder_1ca803(ISheet sheet, string placeholder, object[] data)
        {
            int[,] indexesArray = GetCellIndexByPlaceholder_1ca803(sheet, placeholder);
            for (int i = 0; i < data.Length; i++)
            {
                ...
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, intValue);
                }
                else if (data[i] is long longValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, longValue);
                }
                else if (data[i] is double doubleValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, doubleValue);
                }
                else if (data[i] is decimal decimalValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, Convert.ToDouble(decimalValue));
                }
                else if (data[i] is short shortValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, shortValue);
                }
                else if (data[i] is float floatValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, floatValue);
                }
                else if (data[i] is string stringValue)
                {
                    WriteCellValueByIndex_1ca803(sheet, rowIndex, colIndex, stringValue);
                }
            }
        }
        // 取代資料所有出現1次以上的placeholder
        static void ReplaceAllMultipleVariable_1ca803(ISheet sheet, Dictionary<string, object[]> data)
        {
            string[] uniqueVariables = FindPlaceholderAppearingTimes_1ca803(sheet, 2);
            foreach (string placeholder in uniqueVariables)
            {
                if (data.ContainsKey(placeholder))
                {
                    ReplaceMultiplePlaceholder_1ca803(sheet, placeholder, (object[])data[placeholder]);
                }
            }
        }
        // 依placeholder1(或與placeholder2比較)，依資料筆數調整row數
        static void AdjustRowNums_1ca803(ISheet sheet, Dictionary<string, object[]> data, string placeholder1, string? placeholder2 = null, string? placeholder3 = null)
        {
            string placeholder;
            ...
            if (rowNumDiff >= 0)
            {
                RemoveRangeRows_1ca803(sheet, indexesArray[0, 0] + 1, indexesArray[indexesArray.GetLength(0) - 1, 0]);
                CopyRowDown_1ca803(sheet, indexesArray[0, 0], indexesArray[indexesArray.GetLength(0) - 1, 0] - indexesArray[0, 0] + Math.Abs(rowNumDiff));
            }
        ...
        // 取代資料所有placeholder
        static void ImportDataIntoPlaceholder_1ca803(ISheet sheet, Dictionary<string, object[]> data)
        {
            ReplaceAllSinglePlaceholder_1ca803(sheet, data);
            ReplaceAllMultipleVariable_1ca803(sheet, data);
            ReplacePlaceholdersWithEmpty_1ca803(sheet);
        }
        // 水平合併mergeColRangeArray，依placeholder所在row位置重複執行
        static void HorizontalMergeCols_1ca803(ISheet sheet, string placeholder, int[,] mergeColRangeArray)
        {
            int[,] Index = GetCellIndexByPlaceholder_1ca803(sheet, placeholder);
            foreach (int i in Enumerable.Range(1, Index.GetLength(0) - 1))
            {
                ...
                {
                    MergeCells_1ca803(sheet, Index[i, 0], mergeColRangeArray[j, 0], Index[i, 0], mergeColRangeArray[j, 1]);
                }
            }
        }
        // 垂直合併placeholder所在的row，依mergeCol重複執行
        static void VerticalMergeRows_1ca803(ISheet sheet, string placeholder, int[] mergeCol)
        {
            int[,] Index = GetCellIndexByPlaceholder_1ca803(sheet, placeholder);
            foreach (int i in mergeCol)
            {
                MergeCells_1ca803(sheet, Index[0, 0], i, Index[Index.GetLength(0) - 1, 0], i);
            }
        }
        // 將未被資料取代的placeholder清除
        static void ReplacePlaceholdersWithEmpty_1ca803(ISheet sheet)
        {
            for (int rowIndex = 0; rowIndex <= sheet.LastRowNum; rowIndex++)
            {
                IRow row = sheet.GetRow(rowIndex);
        ...
        static List<int> GetMergedRangeHor_1ca803(ISheet sheet, int rowIndex, int colIndex)
        {
            rowIndex--;
            colIndex--;
            List<int> result = new();
        ...
        static void AdjustRowHeight_1ca803(ISheet sheet, Dictionary<string, object[]> variables, string placeholder1, string? placeholder2 = null, string? placeholder3 = null)
        {
            string placeholder;
            if (!string.IsNullOrEmpty(placeholder1) && !string.IsNullOrEmpty(placeholder2) && !string.IsNullOrEmpty(placeholder3))
            {
                int[,] indexesArray1 = GetCellIndexByPlaceholder_1ca803(sheet, placeholder1);
                int[,] indexesArray2 = GetCellIndexByPlaceholder_1ca803(sheet, placeholder2);
                int[,] indexesArray3 = GetCellIndexByPlaceholder_1ca803(sheet, placeholder3);
                object[] data1 = variables[placeholder1];
                object[] data2 = variables[placeholder2];
                object[] data3 = variables[placeholder3];
                var MergedRangeHor1 = GetMergedRangeHor_1ca803(sheet, indexesArray1[0, 0], indexesArray1[0, 1]);
                var MergedRangeHor2 = GetMergedRangeHor_1ca803(sheet, indexesArray2[0, 0], indexesArray2[0, 1]);
                var MergedRangeHor3 = GetMergedRangeHor_1ca803(sheet, indexesArray3[0, 0], indexesArray3[0, 1]);
            ...
            else
            {
                ...
                var MergedRangeHor = GetMergedRangeHor_1ca803(sheet, indexesArray[0, 0], indexesArray[0, 1]);
        ...
        // 調整row數及row高
        static void AdjustRowNumsAndHeight_1ca803(ISheet sheet, Dictionary<string, object[]> variables, string placeholder1, string? placeholder2 = null, string? placeholder3 = null)
        {
            AdjustRowNums_1ca803(sheet, variables, placeholder1, placeholder2, placeholder3);
            AdjustRowHeight_1ca803(sheet, variables, placeholder1, placeholder2, placeholder3);
        }
    }
}
